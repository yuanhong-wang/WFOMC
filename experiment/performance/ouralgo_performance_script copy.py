...
class Config:
    ...
    GROUPS = [ # 下面是Julia
        ...
        #     "algorithms": ["fast"],
        #     "models": {
        #         "4-regular-graph": None,
        #     }
        # },
        # {
        #     "name": "5-regular-graph",
        #     "domain_size": list(range(2, 100, 3)),
        #     "algorithms": ["fast"],
        #     "models": {
        #         "5-regular-graph": None,
        #     }
        # },
        # {
        #     "name": "3-regular-graph-2-colored",
        #     "domain_size": list(range(2, 100, 3)),
        #     "algorithms": ["fast"],
        #     "models": {
        #         "3-regular-graph-2-colored": None,
        #     }
        # },
        # {
        ...
        #     "algorithms": ["fast"],
        #     "models": {
        #         "3-regular-graph-3-colored": None,
        #     }
        # },
        # {
        #     "name": "3-regular-graph-4-colored",
        #     "domain_size": list(range(2, 100, 1)),
        #     "algorithms": ["fast"],
        #     "models": {
        #         "3-regular-graph-4-colored": None,
        #     }
        # },
        # {
        #     "name": "4-regular-graph-2-colored",
        #     "domain_size": list(range(2, 100, 3)),
        #     "algorithms": ["fast"],
        #     "models": {
        #         "4-regular-graph-2-colored": None,
        #     }
        # },
        # {
        ...
        #     "algorithms": ["fast"],
        #     "models": {
        #         "4-regular-graph-3-colored": None,
        #     }
        # },
        # {
        #     "name": "4-regular-graph-4-colored",
        #     "domain_size": list(range(2, 100, 1)),
        #     "algorithms": ["fast"],
        #     "models": {
        #         "4-regular-graph-4-colored": None,
        #     }
        # },     
        # {
        ...
        #     "algorithms": ["fast"],
        #     "models": {
        #         "5-regular-graph-2-colored": None,
        #     }
        # },     
        {
            "name": "3-regular-directed-graph",
            "domain_size": list(range(2, 100, 3)),
            "algorithms": ["fast"],
            "models": {
                "3-regular-directed-graph": None,
            }
        },
        {
            "name": "2-regular-directed-graph",
            "domain_size": list(range(2, 100, 3)),
            "algorithms": ["fast"],
            "models": {
                "2-regular-directed-graph": None,
            }
        },
        # {
        #     "name": "m-odd-degree-graph-sc2",
        #     "domain_size": list(range(6,18,2)), # 固定n， k 作为变量
        #     # "domain_size": list(range(2,11,1)),
        #     "algorithms": ["dr"],
        #     "models": {
        ...
        # {
        #     "name": "BA",  # 实验组名称
        #     "domain_size": list(range(4, 100, 1)),  # 域大小范围
        #     # "domain_size": list(range(4, 5, 1)),  # 域大小范围
        #     "algorithms": ["dr", "incremental"],  # fast 没有实现Linear order
        #     "models": {  # 要测试的模型
        #         "BA_CC": "BA_CC.wfomcs",
        #         "BA": "BA.wfomcs",
        #     },
...
# 因为运行Julia的代码 底层用到的c++库会和symengine中的某一个的一个库 产生冲突，于是这里注释掉，等到了使用的时候，才导包
# try:
...
def python_worker(q, model_name, model_csv_name, domain_size, algo):
    ...
    try:
        ...
        #     my = str(domain_size)
        #     final_model_string = model_template_string.replace('=m', f"={my}")
            ...
            if (
                ...
                for old_constraint in problem.cardinality_constraint.constraints:
                    new_constraint = (
                        old_constraint[0],
                        old_constraint[1],
                        domain_size,  # 这里默认基数约束和domain_size 是相同的。# 根据domain_size 构造新的constraints
                    )  # 假设约束的第三个参数（通常是数量）与域大小相关
...
# 这里是不包含Julia的原始版本
def run_python_single(model_name, model_csv_name, domain_size, algo, r=None, k=None):
    """
    ...
    p = Process(target=python_worker, args=(q, model_name, model_csv_name, domain_size, algo))
    p.start()
    # 创建并启动内存监控线程
    peak_mem = mp.Value(
        "Q", 0
                ...
                with open(csv_path, "w", newline="") as csvfile:  # 打开结果文件准备写入
                    ...
                    for algo in [a for a in Algo if str(a) in algorithms]:
                        ...
                        for n in domain_size:  # 遍历所有域大小
                            ...
                            # 运行算法
                            global use_julia
                            if use_julia and str(algo)=="fast":
                                single_result = run_julia_single(
                                    model_name, n, "fast"
                                )  # 运行单个实验，读取model文件，运行wfomc，返回结果
                            else:
                                single_result = run_python_single(
                                    model_name, model_csv_name, n, algo
                                )  # 运行单个实验，读取model文件，运行wfomc，返回结果
