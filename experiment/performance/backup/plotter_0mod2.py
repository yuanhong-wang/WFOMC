...
def plot_performance(csv_file_name: str, metric: str, ylabel: str, output_suffix: str):
    """
    通用的绘图函数，用于绘制算法性能对比图
    ...
    # domain_sizes = list(range(2, 40, 2))
    ...
    for algo in algorithms:
        ...
        # 根据算法确定 domain_sizes 这里是为了缩小输出图表的范围
        # if algo == "dr":
        #     # 为 "incremental" 算法指定 domain size
        #     domain_sizes = list(range(4, 100, 7))
        # else:
        ...
        plt.plot(domain_sizes, values, marker=markers.get(algo, "o"),
                 ...
                 color=colors.get(algo, 'black'),  # 使用我们
                 markersize=12,
                 linewidth=3,
    ...
    from matplotlib import ticker  # 1. 在这里增加导入
...
def plot_runtime_comparison(csv_file_name: str):
    ...
    """
    plot_performance(csv_file_name, 'time_sec',
                     'Runtime (s)', 'time_comparison')
...
def plot_memory_comparison(csv_file_name: str):
    ...
    """
    plot_performance(csv_file_name, 'memory_mb',
                     'Memory Usage (MB)', 'memory_comparison')


if __name__ == '__main__':
    RESULT_PATH = os.path.join(os.path.abspath("../"), "results", "rmodk")
    # plot_runtime_comparison("baseline_0mod2-regular-graph.csv")
    plot_runtime_comparison("baseline_1mod2-regular-graph.csv")
    plot_runtime_comparison("baseline_2mod4-regular-graph.csv")
