...
class Config:
    ...
    RESULTS_PATH = None  # 只声明变量，不在这里创建文件夹
    
    MODELS_PATH = "/home/sunshixin/pycharm_workspace/WFOMC/models"  # 输入例子的路径
    # 为每次执行创建带时间戳的结果目录
...
def plot_metric(
    ...
    plot_comparison(csv_file_path, metric, ylabel, suffix)
...
def run_python_single(model_name, model_csv_name, domain_size, algo, r=None, k=None):
    ...
    p = Process(target=python_worker, args=(
        q, model_name, model_csv_name, domain_size, algo))
    p.start()
    # 创建并启动内存监控线程
    peak_mem = mp.Value(
        "Q", 0
...
def run_experiment(FLUSH_EVERY: int = 20):
    ...
    for group in Config.GROUPS:
        ...
        group_iterations = len(domain_size) * \
            len(algorithms) * len(group["models"])
                ...
                with open(csv_path, "w", newline="") as csvfile:  # 打开结果文件准备写入
                    ...
                    for algo in [a for a in Algo if str(a) in algorithms]:
                        ...
                        for n in domain_size:  # 遍历所有域大小
                            ...
                            if (
                                single_result["status"] == "timeout" or single_result[
                                    "status"] == "hard_timeout" or single_result["status"] == "error"
                            ):  # 如果任何一次运行超时，则标记跳过该算法
...
if __name__ == "__main__":
    ...
    timestamp = datetime.now(ZoneInfo('Asia/Shanghai')
                             ).strftime('%Y%m%d_%H%M%S')  # 获取当前时间戳
    Config.RESULTS_PATH = os.path.join(
        Config.DIR_PATH, f"results_{timestamp}")  # 存储实验结果的路径的文件夹路径，但不是最终csv结果的路径
    os.makedirs(Config.RESULTS_PATH, exist_ok=True)  # 确保结果目录存在
    print(f"实验结果将保存在: {Config.RESULTS_PATH}")  # 打印结果目录路径
    FLUSH_EVERY = 1  # 每 1 条记录刷新一次
    run_experiment(FLUSH_EVERY)  # 执行实验
