...
def branch_applies(branch: object, domain: Domain) -> bool:
    if not isinstance(branch, CompiledReducedProblem):
        raise TypeError("Recursive compiled branch has an invalid type")
...
def instantiate_branch(
    ...
    if not isinstance(branch, CompiledReducedProblem):
        raise TypeError("Recursive compiled branch has an invalid type")
    if not isinstance(input_template, RecursiveInputTemplate):
        raise TypeError("Recursive input template has an invalid type")
