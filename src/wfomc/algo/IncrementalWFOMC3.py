...
from wfomc.utils import MultinomialCoefficients, expand, RingElement
...
class ConfigUpdater:
    ...
    def __init__(self, t_update_dict, space: ConfigSpace):
...
def _make_domain_recursion(
    t_update_dict, space: ConfigSpace, cs: CountingState, has_linear_order: bool
) -> Callable[[Config], RingElement]:
    """Return a memoised domain_recursion function scoped to one cell graph."""
    updater = ConfigUpdater(t_update_dict, space)
    f = updater.f
