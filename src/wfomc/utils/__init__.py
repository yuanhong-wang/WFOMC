...
from .polynomial_flint import RingElement, to_ringelements, to_symexpr
...
def coeff_dict(p: Poly, gens: list[Expr]) \
        -> Generator[tuple[int], Expr, None]:
    p = Poly(p, gens)
    for degrees, coeff in p.terms():
        yield degrees, coeff
