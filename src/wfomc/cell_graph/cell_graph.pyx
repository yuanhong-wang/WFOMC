...
class OptimizedCellGraph(CellGraph):
    ...
    def get_term(self, iv: int, bign: int, partition: tuple) -> object:
        ...
        if (iv_int, bign_int) in self.term_cache:
            return self.term_cache[(iv_int, bign_int)]
        ...
        if iv_int == 0:
            ...
            for j in self.i1_ind:
                ...
                accum = accum + tmp
            accum = accum ** (self.domain_size - sum(partition) - bign_int)
            self.term_cache[(iv_int, bign_int)] = accum
            return accum
        else:
            sumtoadd = 0
            s = self.i2_ind[len(self.i2_ind) - iv_int]
            for nval in range(self.domain_size - sum(partition) - bign_int + 1):
                smul = MultinomialCoefficients.comb(
                    self.domain_size - sum(partition) - bign_int, nval
                )
                ...
                smul = smul * self.get_term(
                    iv_int - 1, bign_int + nval, partition
                )
                sumtoadd = sumtoadd + smul
            self.term_cache[(iv_int, bign_int)] = sumtoadd
            return sumtoadd
