...
def test_parse_minimal_problem_uses_typed_defaults():
    ...
    assert isinstance(parsed, ProblemInstance)
    assert isinstance(parsed.problem, Problem)
    assert isinstance(parsed.domain, Domain)
    assert isinstance(parsed.problem.sentence, Formula)
    assert parsed.problem.sentence.op is FormulaKind.FORALL
    assert parsed.domain.elements == frozenset()
    assert parsed.problem.weights == {}
    assert parsed.problem.cardinality_constraints == CardinalityConstraints()
    assert parsed.problem.evidence.unary.is_empty
...
"""
    ...
    quantified_variable, body = parsed.problem.sentence.args
    predicate, term = body.args
    literal = parsed.problem.evidence.unary.literals[0]

    assert isinstance(quantified_variable, Variable)
    assert isinstance(predicate, Predicate)
    assert isinstance(term, Variable)
    assert all(
        isinstance(constant, Constant) for constant in parsed.domain.elements
    )
    assert parsed.problem.weights[predicate] == (2, 3)
...
"""
    ...
    constraint = parsed.problem.cardinality_constraints.constraints[0]
...
def test_parse_mln_file_preserves_weights_constraints_and_evidence():
    ...
    assert len(parsed.problem.weights) == 1
    constraint = parsed.problem.cardinality_constraints.constraints[0]
...
def _evidence_strings(instance: ProblemInstance) -> set[str]:
    return {
        ...
        for literal in instance.problem.evidence.unary.literals
    }
